...
*/
  ...
  while((left < THRESHOLD_LEFT) && (right < THRESHOLD_RIGHT) && (distance > 20))
  {
    robot.forward(SPEED);
    readlight();
    distance = sonarDistance();
  ...
  while(distance < 20)
  {
    ...
    distance = sonarDistance();
    readlight();
  }//Stop when there is an object

  while((left > THRESHOLD_LEFT) && (right < THRESHOLD_RIGHT))
  {
    robot.rotateCCW(TURN_SPEED);
    readlight();
  }//Left is on the black correct
